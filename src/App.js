...
function Home(args) {
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:3000/employees') // Replace with your actual API endpoint
  ...
  function EmployeeHierarchy() {
    ...
    useEffect(() => {
      ...
      fetch('http://localhost:3000/employeeHierarchy') // Replace with your actual API endpoint
        .then((response) => response.json())
        .then((data) => setHierarchicalData(data))
        .catch((error) => console.error('Error fetching hierarchical data:', error));
    }, []);
      ...
      <div>
        ...
        <JSONTree data={hierarchicalData}
         labelRenderer={(keyPath, nodeType, expanded, expandable) => (
          <span style={{ fontWeight: 'bold' }}>{keyPath.join('.')}</span>
        )}
  ...
  return (
    <div>
      ...
      <div className='add-employee'>
        <h3>Add Employee</h3>
        <br></br>
  <Button color='primary' size='lg' onClick={toggle} >Add Employee</Button>
